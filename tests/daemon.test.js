...
test('a first start writes the default chain and reports the missing keys', async () => {
  ...
  try {
    ...
    assert.equal(await fs.access(runtimePathFor(box.file)).then(() => true, () => false), false, 'the runtime file is cleaned up');
  } finally {
    await box.cleanup();
  }
});
...
test('the published bundle runs the service from a copy, and cleans up when it is gone', bundled, async () => {
  ...
  try {
    ...
    await fs.writeFile(
      path.join(box.dir, 'service', 'origin.json'),
      JSON.stringify({ origin: path.join(box.dir, 'uninstalled.js') }),
    );
...
test('the login entry points at this install and can be removed', async () => {
  ...
  try {
    ...
    assert.equal(await fs.access(target.file).then(() => true, () => false), false);
