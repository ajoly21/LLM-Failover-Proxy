...
export function SetupScreen({ config, update, notify, onDone, catalog: injected }) {
  ...
  return h(
    Frame,
    {
      ...
      footer: h(Hints, {
        items: [
          ['↑↓', 'move'],
          ['enter', 'choose'],
          choices.length > 1 ? [`1-${choices.length}`, 'jump'] : null,
          ['esc', 'skip'],
        ],
      }),
    },
    ...
    catalog.error
      ? h(Box, { paddingTop: 1 }, h(Text, { color: COLOR.fail }, `  default chain unavailable: ${catalog.error}`))
      : h(ChainPreview, { catalog }),
  );
}
...
function ChainPreview({ catalog }) {
  ...
  return h(
    ...
    ...shown.map((entry, position) =>
      h(
        Text,
        { key: entry.model, dimColor: true },
        `   ${String(position + 1).padStart(2)}. `,
        h(Text, { color: COLOR.accent }, entry.provider),
        `/${entry.model}`,
      ),
    ),
  ...
  return h(
    ...
    h(
      ...
      current.models.length
        ? h(Text, { dimColor: true, wrap: 'truncate' }, `  used by: ${current.models.join(', ')}`)
        : null,
    ),
    h(
      Box,
    ...
    h(
      Box,
      { paddingTop: 1 },
      h(Text, { dimColor: true }, '  skipping is fine: the chain steps over a provider with no key'),
    ),
  );
}
  ...
  return h(
    ...
    h(
      ...
      h(
        ...
        `${added.models.length} model(s) and ${added.providers.length} provider(s) added`,
      ),
      ...
      written.length
        ? h(
            ...
            null,
            h(Text, { color: COLOR.ok }, `  ${SYMBOL.ok} `),
            `${written.length} key(s) saved to `,
            h(Text, { dimColor: true }, envFile),
          )
        : null,
      ...
      ...missing.map((entry) =>
        h(
          Text,
          { key: entry.envVar, color: COLOR.warn },
          `  ${SYMBOL.off} ${entry.name}: no key — set ${entry.envVar}${entry.keyUrl ? ` (${entry.keyUrl})` : ''}`,
        ),
      ),
    ),
    h(
