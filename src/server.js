...
import { enableStatsPersistence, snapshot, stateFor, statsFile as currentStatsFile, statsSince } from './state.js';
...
function unauthorized(res, config) {
  sendJson(res, config, 401, openAIError('Missing or invalid proxy API key.', {
...
export function createServer({ configFile, statsFile } = {}) {
  ...
  const reload = (why) => {
    try {
      ...
      setLogLevel(config.server.logLevel);
      log.info(
        c.cyan('config reloaded'),
        c.gray(`(${why})`),
        `— ${config.models.length} model(s), ${config.providers.length} provider(s)`,
      );
    } catch (err) {
    ...
    try {
      ...
      if (req.method === 'POST' && (endpoint === '/chat/completions' || endpoint === '/embeddings')) {
        ...
        try {
          ...
          sendJson(res, config, err.status || 400, openAIError(`Invalid request body: ${err.message}`, {
            type: 'invalid_request_error',
          }));
          return;
        }
        if (kind === 'chat' && !Array.isArray(body.messages)) {
          sendJson(res, config, 400, openAIError('The `messages` field is required.', {
            type: 'invalid_request_error',
            param: 'messages',
          }));
          return;
        ...
        log.info(
          `[${requestId}] ${c.bold(kind === 'embedding' ? 'embeddings' : 'chat')} model=${c.cyan(body.model || 'auto')}` +
            `${wantsStream ? c.gray(' stream') : ''}`,
        );
      ...
      sendJson(res, config, 404, openAIError(`Unknown route: ${req.method} ${route}`, {
        type: 'invalid_request_error',
        code: 'unknown_route',
      }));
    } catch (err) {
...
export async function startServer({ configFile, statsFile, port, host } = {}) {
  ...
  log.raw(
    `  ${c.gray('proxy API key  ')}  ${resolveSecret(config.server.apiKey) ? 'required' : c.yellow('not required (open on localhost)')}`,
  );
